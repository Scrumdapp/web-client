  ...
  return {
    ...
    impediment:
      Math.random() > 0.6
        ? undefined
        : RANDOM_IMPEDIMENT[
            Math.floor(Math.random() * RANDOM_IMPEDIMENT.length)
          ],
...
export const groupCheckpointHandlers = [
  http.get("/api/groups/:gid/sessions", ({ params, request }) => {
    ...
    const from = parseScrumdappDate(
      (url.searchParams.get("start_date") as string | null) ?? "1970-01-01",
    ).getTime();
    const to = parseScrumdappDate(
      (url.searchParams.get("end_date") as string | null) ?? "1970-01-01",
    ).getTime();
    const date = parseScrumdappDate(
      (url.searchParams.get("date") as string | null) ?? "1970-01-01",
    ).getTime();
  ...
  http.get("/api/groups/:gid/sessions/dates", ({ params, request }) => {
    ...
    const uniqueDates = new Set<string>(
      sessions.map((it) => toScrumdappDate(new Date(it.sessions.startTime))),
    );
    ...
    if (url.searchParams.has("month")) {
      ...
      dates = dates.filter((it) => {
        const d = parseScrumdappDate(it).getTime();
    ...
    return HttpResponse.json<SessionDates>({
      ...
      dates: dates.map((it) => ({
        ...
        sessions: groupCheckpoints
          .filter(
            (cp) => toScrumdappDate(new Date(cp.sessions.startTime)) == it,
          )
