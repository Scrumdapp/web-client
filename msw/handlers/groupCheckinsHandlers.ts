...
import {
  parseScrumdappDate,
  toScrumdappDate,
} from "../../src/js/utils/scrumdappDate.ts";
...
function generateCheckin(
  ...
  if (features.indexOf("checkins.presence") != -1) {
    checkin.presence =
      Math.random() > 0.7
        ? undefined
        : PRESENCE_FIELDS[Math.floor(Math.random() * PRESENCE_FIELDS.length)];
  }

  if (features.indexOf("checkins.checkin_stars") != -1) {
    checkin.checkin_stars =
      Math.random() > 0.7 ? undefined : Math.floor(Math.random() * 11);
  }

  if (features.indexOf("checkins.checkup_stars") != -1) {
    checkin.checkup_stars =
      Math.random() > 0.7 ? undefined : Math.floor(Math.random() * 11);
  }

  if (features.indexOf("checkins.checkout_stars") != -1) {
    checkin.checkout_stars =
      Math.random() > 0.7 ? undefined : Math.floor(Math.random() * 11);
  }

  if (features.indexOf("checkins.presence_comment") != -1) {
    checkin.presence_comment =
      RANDOM_COMMENT[Math.floor(Math.random() * RANDOM_COMMENT.length)];
  }
...
function parseCheckinFields(
  ...
  for (let i = 0; i < all.length; i++) {
    switch (all[i].toLowerCase()) {
      case "presence":
        r["presence"] = checkin.presence ?? null;
        break;
      case "presence_comment":
        r["presence_comment"] = checkin.presence_comment ?? null;
        break;
      case "checkin_stars":
        r["checkin_stars"] = checkin.checkin_stars ?? null;
        break;
      case "checkin_comment":
        r["checkin_comment"] = checkin.checkin_comment ?? null;
        break;
      case "checkup_stars":
        r["checkup_stars"] = checkin.checkup_stars ?? null;
        break;
      case "checkup_comment":
        r["checkup_comment"] = checkin.checkup_comment ?? null;
        break;
      case "checkout_stars":
        r["checkout_stars"] = checkin.checkout_stars ?? null;
        break;
      case "checkout_comment":
        r["checkout_comment"] = checkin.checkout_comment ?? null;
        break;
      case "obstacle_comment":
        r["obstacle_comment"] = checkin.obstacle_comment ?? null;
        break;
    }
  }
...
export const groupCheckinsHandlers = [
  http.get("/api/groups/:gid/users/:uid/checkins", ({ params, request }) => {
    ...
    // @ts-ignore
    const checkins = groupCheckins.filter(
      (it) =>
        it.user_id == params.uid &&
        it.group_id == params.gid &&
        parseScrumdappDate(it.date) >= from &&
        parseScrumdappDate(it.date) <= to,
    );
    return HttpResponse.json(
      checkins.map((it) => parseCheckinFields(fields, it)),
    );
  }),
  http.get(
    "/api/groups/:gid/users/:uid/checkins/:date",
    ({ params, request }) => {
      const url = new URL(request.url);
      const date = url.searchParams.get("date");
      const fields = url.searchParams.get("fields");
      // @ts-ignore
      const checkin = groupCheckins.find(
        (it) =>
          it.user_id == params.uid &&
          it.group_id == params.gid &&
          it.date == date,
  ...
  http.patch("/api/groups/:gid/users/:uid/checkins/:date", ({ params }) => {
    // @ts-ignore
    const checkins = groupCheckins.find(
      (it) =>
        it.user_id == params.uid &&
        it.group_id == params.gid &&
        it.date == params.date,
  ...
  http.get("/api/groups/:gid/checkins/:date", ({ params, request }) => {
    ...
    // @ts-ignore
    const checkins = groupCheckins.filter(
      (it) => it.group_id == params.gid && it.date == params.date,
    );
    return HttpResponse.json(
      checkins.map((it) => parseCheckinFields(fields, it)),
    );
