...
export function InviteRouter() {
  ...
  const getUserGroups = useApi(ScrumdappApi.getGroups(), {
    fetchOnCreated: [],
  });
  ...
  const group = getUserGroups.data!.find(
    (it) => it.id == getGroupInvite.data?.groupId,
  );
  if (group) {
    return <InGroup group={group} />;
  }

  return <AcceptInvite invite={getGroupInvite.data!} />;
}
