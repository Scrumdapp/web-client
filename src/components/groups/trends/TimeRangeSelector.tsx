...
export function TimeRangeSelector({
  ...
  const options = [
    {
      ...
      from: () =>
        getWeekStart(new Date(new Date().getTime() - 14 * 24 * 60 * 60 * 1000)),
      to: () => new Date(),
    },
    {
      ...
      from: () =>
        getWeekStart(
          new Date(new Date().getTime() - 6 * 4 * 7 * 24 * 60 * 60 * 1000),
        ),
      to: () => new Date(),
    },
  ];
  ...
  const selectOption = (index: number) => {
    ...
    if (onRangeSelected) {
      ...
      onRangeSelected(
        toScrumdappDate(option.from()),
        toScrumdappDate(option.to()),
      );
    }
  };
  ...
  return (
    <Menu as="div" className="relative">
      ...
      <MenuItems
        modal={false}
        transition
        className="absolute z-10 mt-2 border rounded-md bg-bg w-full"
      >
        {options.map((it, i) => (
          <MenuItem
            key={i}
