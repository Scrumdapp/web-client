...
export const GroupTimelineTrends = memo(
  ({ users, from, to, display }: GroupTimelineTrendsProps) => {
    const { t } = useTranslation();
    const getGroupTimelineTrends = useApi(
      ScrumdappApi.getGroupTimelineTrends(),
    );
    ...
    if (getGroupTimelineTrends.loading) {
      ...
      component = (
        <ErrorScreen error={new ApiError(999, "No trends available")} />
      );
    } else if (display == GroupTimelineDisplayType.Cumulative) {
      component = (
        <RenderCumulativeGraph
          users={users}
          data={getGroupTimelineTrends.data}
        />
      );
    } else {
      component = (
        <RenderTimelineGraph users={users} data={getGroupTimelineTrends.data} />
      );
    }
    ...
    return (
      <>
        <div
          className="vertical"
          style={{ height: getGroupTimelineHeight(users) }}
        >
          {component}
        </div>
          ...
          {attendanceOptions.map((it) => (
            <div className="horizontal gap-2 items-center">
              ...
              <p className="text-fg2">
                {t(it.labelKey) === "---"
                  ? t("checkpoint.attendanceOptions.noData")
                  : t(it.labelKey)}
              </p>
            </div>
          ))}
